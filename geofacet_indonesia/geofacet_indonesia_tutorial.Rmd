...
This is a tutorial about [geofacet](https://hafen.github.io/geofacet) Indonesia.
I use presidential election as a use case. You can use following code for other use case as long as it base on province in Indonesia.
...
We will create a map based on this picture. I did some adjusment to save more space
![map indonesia](indonesia_geofacet_ai.png)
```{r create a map, echo=FALSE}
...
## Get Presidential Election Result by Province

- [2014 Presidential Election Result](https://nasional.kompas.com/read/2014/07/22/18235991/Ini.Hasil.Rekapitulasi.Suara.Pilpres.di.33.Provinsi?page=all)
- [2019 Presidential Election Result](https://nasional.kompas.com/read/2019/05/21/05050031/ini-hasil-lengkap-rekapitulasi-suara-pilpres-2019-di-34-provinsi?page=all)

After some cleansing from that source we get this
...
## Tidying the data so can be used
```{r df long version}
...
## First Plot: Result from 2019 Presidential Election
```{r plot result 2019 presidential election, fig.height = 6, fig.width = 10, fig.align = "center"}
...
## Second Plot: % Changed 2014-2019 Presidential Election
```{r plot result 2014-2019 presidential election, fig.height = 7, fig.width = 11, fig.align = "center"}
pilpres <- pilpres_long %>% rename(name_indo = name)

ggplot(data=pilpres,aes(x=as.factor(year),y=pct_votes,group=candidate))+
  geom_line(aes(color = candidate))+
  geom_point(aes(color = candidate))+
  geom_text_repel(data = pilpres %>% filter(year == 2014), 
            aes(label = paste0(round(pct_votes*100,1), "%")) , 
            #hjust = -1, 
            fontface = "bold",
            size=1.4)+
  geom_text_repel(data = pilpres %>% filter(year == 2019), 
            aes(label = paste0(round(pct_votes*100,1), "%")) , 
            #hjust = -1, 
            fontface = "bold",
            size=1.4)+
